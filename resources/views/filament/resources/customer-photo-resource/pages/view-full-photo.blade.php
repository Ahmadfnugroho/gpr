        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            <div class="mb-4">
                <h3 class="text-lg font-semibold text-gray-900">
                    Foto {{ $customerPhoto->photo_type ?: 'Dokumen' }} - {{ $customerPhoto->customer->name }}
                </h3>
                <p class="text-sm text-gray-600 mt-1">
                    Diupload: {{ $customerPhoto->created_at?->format('d M Y H:i') }}
                </p>
                @if($customerPhoto->id_type)
                <p class="text-sm text-gray-600">
                    ID Type: {{ $customerPhoto->id_type }}
                </p>
                @endif
            </div>

            <div class="text-center">
                @if($customerPhoto->photo)
                <img
                    src="{{ Storage::disk('public')->url($customerPhoto->photo) }}"
                    alt="Foto {{ $customerPhoto->photo_type ?: 'Dokumen' }}"
            ...
            <div class="mt-4 text-center">
                @if($customerPhoto->photo)
                <a
                    href="{{ Storage::disk('public')->url($customerPhoto->photo) }}"
                    target="_blank"
